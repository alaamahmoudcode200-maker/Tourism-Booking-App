  ...
  return (
    <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-4 sm:mb-6 gap-2 sm:gap-4">
        ...
        <a href="#" className="text-sm font-medium text-gray-900 hover:text-orange-600 transition">see all</a>
      </div>
      <p className="text-sm text-gray-500 mb-6 sm:mb-8">Favorite destinations based on customer reviews</p>
        ...
        {tours.map((t) => (
          ...
          >
            <div className="relative h-[200px] sm:h-[240px] overflow-hidden">
              ...
              <span className={`absolute top-3 sm:top-4 left-3 sm:left-4 text-white text-xs font-semibold px-2 sm:px-3 py-1 rounded-full ${t.badge.color}`}>{t.badge.text}</span>
              ...
              >
                <FaRegHeart className={`text-xs sm:text-sm ${fav[t.id] ? "text-red-500" : "text-gray-600"}`} />
              </button>
            </div>
            <div className="absolute bottom-[-100%] left-0 w-full bg-white p-4 sm:p-5 transition-all duration-500 group-hover:bottom-0">
              <h3 className="text-base sm:text-lg text-gray-700 font-semibold mb-2 sm:mb-3 line-clamp-2">{t.title}</h3>
              <div className="flex flex-col sm:flex-row text-xs sm:text-sm text-gray-600 gap-2 sm:gap-6 mb-3 sm:mb-4">
                <span className="flex items-center gap-2"><FaMoon className="text-xs sm:text-sm" /> {t.duration}</span>
                <span className="flex items-center gap-2"><FaUser className="text-xs sm:text-sm" /> {t.guests}</span>
              </div>
              ...
              <div className="flex items-center justify-between">
                ...
                <button className="px-3 sm:px-4 py-2 bg-[#E85528] text-white rounded-md font-semibold hover:bg-[#c9441e] text-xs sm:text-sm transition">Book Now</button>
              </div>
            </div>
          </article>
        ))}
